...
def main():
    ...
    with open("pet_train/pet_train.pkl", "rb") as fh:
        data = pickle.load(fh)
    X_train_ = pd.DataFrame(data)["pet_img_array"] 
    
    with open("pet_train/pet_test.pkl", "rb") as fh:
        data = pickle.load(fh)
    X_test_ = pd.DataFrame(data)["pet_img_array"]
    
    with open("pet_train/pet_y_train.pkl", "rb") as fh:
        data = pickle.load(fh)
    y_train = np.array(pd.DataFrame(data)["pet_label"].values.astype(np.float32)).flatten()
    
    with open("pet_train/pet_y_test.pkl", "rb") as fh:
        data = pickle.load(fh)
    y_test = np.array(pd.DataFrame(data)["pet_label"].values.astype(np.float32)).flatten()
