...
def train(mode, batch_size, epochs, learning_rate):
    ...
    #?clinical
    train_clinical = pd.read_pickle("X_train_clinical.pkl").fillna(0).replace(r'[^0-9]',0,regex=True).astype("float32").values
    # test_clinical= pd.read_pickle("overlap_train/X_test_clinical.pkl").fillna(0).replace(r'[^0-9]',0,regex=True).astype("float32").values

    #?Genetic
    train_snp = pd.read_pickle("X_train_snp_M.pkl").fillna(0).replace(r'[^0-9]',0,regex=True).astype("float32").values
    ...
    #?PET image
    train_pet= make_img("X_train_pet.pkl")
    ...
    #?overlap
    train_label= pd.read_pickle("y_train.pkl").values.astype("float32").flatten()
